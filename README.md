...
### 插件化系统
- **插件发布**：院校模块可通过GitHub Release进行插件化分发
- **单一标签**：所有插件发布到固定的 `plugin/latest` 标签
- **ZIP命名**：插件包格式为 `school_[code]_plugin.zip`，不包含版本号
- **时间戳版本**：版本号使用构建时的时间戳（如 20260130_153022）
- **自动更新**：支持通过GUI界面一键检查和更新插件
