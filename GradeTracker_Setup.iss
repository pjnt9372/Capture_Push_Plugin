; -- GradeTracker_Setup.iss --
; 学业助手安装脚本（内置Python环境，无需系统Python）
...
OutputDir=Output
OutputBaseFilename=GradeTracker_Setup
...
Source: "dist\GradeTracker_Installer.exe"; DestDir: "{app}"; Flags: ignoreversion solidbreak
...
Name: "{group}\卸载学业助手"; Filename: "{uninstallexe}"
Name: "{commondesktop}\学业助手"; Filename: "{app}\TrayApp.exe"; Tasks: desktopicon
Name: "{commonstartup}\学业助手"; Filename: "{app}\TrayApp.exe"; Tasks: autostart
...
[Registry]
; 注册安装路径
Root: HKLM; Subkey: "SYSTEM\CurrentControlSet\Control\Session Manager\Environment"; ValueType: expandsz; ValueName: "GradeTrackerPath"; ValueData: "{app}"; Flags: uninsdeletevalue

; 自启动托盘程序（如果用户选择了autostart任务）
Root: HKLM; Subkey: "SOFTWARE\Microsoft\Windows\CurrentVersion\Run"; ValueType: string; ValueName: "GradeTrackerTray"; ValueData: """{app}\TrayApp.exe"""; Flags: uninsdeletevalue; Tasks: autostart

[Run]
; 运行环境安装器（命令行模式，自动检测地区并选择镜像）
Filename: "{app}\GradeTracker_Installer.exe"; Parameters: """{app}"""; StatusMsg: "正在配置Python环境..."; Flags: waituntilterminated
...
; 安装后选项
Filename: "{app}\TrayApp.exe"; Description: "启动学业助手托盘程序"; Flags: nowait postinstall skipifsilent
